...
import { ThemeCarouselProps } from "@/types/Dashboard-type";

export default function ThemeCarousel({ title, items, className }: ThemeCarouselProps) {
  const scrollRef = useRef<HTMLDivElement | null>(null);
  const trackRef  = useRef<HTMLDivElement | null>(null);

  const [step, setStep] = useState(160);
  ...
  useEffect(() => {
    ...
    const onResize = () => { measureStep(); updateArrows(); };
  ...
  return (
    <section className={clsx("rounded-xl", className)}>
      ...
      >
        <div ref={trackRef} className="flex gap-2 pr-2">
          {items.map((label, idx) => (
            <ThemeTile key={idx} label={label} />
          ))}
        </div>
      </div>
    </section>
